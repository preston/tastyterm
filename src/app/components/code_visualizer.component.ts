...
@Component({
	...
	templateUrl: "../views/code_visualizer.html"
})
export class CodeVisualizerComponent implements OnInit {
	@Input() valueSet: ValueSet | null = null;
	@Input() codeSystem: CodeSystem | null = null;
	@Input() valueSetParameters: Parameters | null = null;
	...
	graph: { links: any[]; nodes: any[] } | null = null;
	...
	orientations: { label: string, value: string }[] = [
		{
			label: "Left to Right",
			value: "LR"
		},
	...
	id(): string {
		...
		if (!this.cache.get(newId)) {
			this.cache.set(newId, true);
			return newId;
		}
