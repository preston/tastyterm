...
export class LaunchComponent implements OnInit {
	...
	ngOnInit() {
		this.http.get(this.conformanceUri, this.tastyTermService.requestOptions(false))
			.subscribe((response: any) => {
				console.log(response);
				let authUri, tokenUri = '';
				let smartExtension = response.rest[0].security.extension.filter((e: any) => {
					return (e.url === "http://fhir-registry.smarthealthit.org/StructureDefinition/oauth-uris");
				});
				smartExtension[0].extension.forEach((arg: any) => {
	...
	protected getUrlParameter(sParam: string): string {
